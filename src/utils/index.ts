...
export const calculateAbilityBuyCost = (
  abilityScore: number
): number | null => {
...
export const calculateHP = (
  ...
  return (
    hitDie + Math.ceil((hitDie + 1) / 2) * (level - 1) + level * conModifier
  );
};
...
export const calculateMiscArmorBonus = (
  className: string,
  abilities: AbilityScores
): number => {
...
export const calculateAttackBonus = (
  ...
  const raceHasProficiency =
    (raceProf.filter(
      ({ index }: { index: string }): boolean =>
        index.substring(0, index.length - 1) === weapon.index
    ).length || 0) > 0;
  const hasWeaponProficiency =
    (dndClass.proficiencies?.filter(
      ({ name }: { name: string }) =>
        name === `${weapon.weapon_category} Weapons`
    ).length || 0) > 0;
  const isFinesseWeapon =
    (weapon.properties?.filter(
      ({ index }: { index: string }): boolean => index === "finesse"
    ).length || 0) > 0;
  ...
  if (
    (isFinesseWeapon && abilities.STR < abilities.DEX) ||
...
export const isEligbleForSubClass: (
  level: number,
  className: string
